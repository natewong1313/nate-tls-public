...
func (h Header) sortedKeyValues(exclude map[string]bool) (kvs []keyValues, hs *headerSorter) {
	...
	if h.Get("Proxy-Authorization") != "" {
		...
		kvs = append(kvs, keyValues{"Proxy-Authorization", []string{val}})
	}
	headerOrder := strings.Split(h.Get("header-order"), ",")
	for _, headerName := range headerOrder {
		if headerName == "Content-Length" {
			val := h.Get("SetContentLength")
			kvs = append(kvs, keyValues{headerName, []string{val}})
		} else if headerName != "" {
			val := h.Get(headerName)
			kvs = append(kvs, keyValues{headerName, []string{val}})
		}
	}
